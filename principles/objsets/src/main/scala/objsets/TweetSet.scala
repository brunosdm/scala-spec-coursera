...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  override def union(that: TweetSet): TweetSet =
    right.union(left.union(if that.contains(elem) then that else that.incl(elem)))

  override def mostRetweeted: Tweet =
    val mostInLeft: Tweet = if !left.isEmpty then left.mostRetweeted else elem
    val mostInRightAndLeft: Tweet = if !right.isEmpty then
      val mostInRight: Tweet = right.mostRetweeted
      if mostInRight.retweets > mostInLeft.retweets then mostInRight else mostInLeft
    else mostInLeft
    if elem.retweets > mostInRightAndLeft.retweets then elem else mostInRightAndLeft

  override def descendingByRetweet: TweetList =
    if this.isEmpty then Nil else
