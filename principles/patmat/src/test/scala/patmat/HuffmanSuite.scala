...
class HuffmanSuite extends munit.FunSuite:
  ...
  test("test times empty list") {
    assertEquals(times(List()), Nil)
  }

  test("test times non-empty list") {
    assertEquals(times(List('a', 'a', 'a', 'b')), List(('a', 3), ('b', 1)))
  }
  ...
  test("decode and encode a larger text") {
    val str: String = "abcdefghaaaaabb"
    val t1: CodeTree = createCodeTree(string2Chars(str))

    assertEquals(decode(t1, encode(t1)(str.toList)), str.toList)
  }

  test("decode and encode a larger text with quickEncode") {
    val str: String = "abcdefghaaaaabb"
    val t1: CodeTree = createCodeTree(string2Chars(str))

    assertEquals(decode(t1, quickEncode(t1)(str.toList)), str.toList)
  }
